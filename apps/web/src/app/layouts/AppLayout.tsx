...
export function AppLayout() {
  return (
    <div className="h-screen overflow-hidden bg-paper text-ink">
      <div className="grid h-full min-h-0 grid-cols-[88px_1fr] lg:grid-cols-[232px_1fr]">
        <aside className="min-h-0 overflow-y-auto border-r border-white/80 bg-cream/90 shadow-[18px_0_48px_rgba(80,52,31,0.10)] backdrop-blur-xl">
          <div className="flex h-[72px] items-center justify-center border-b border-latte/70 px-3 lg:justify-start lg:px-4">
          ...
          <nav className="flex flex-col gap-2 p-2.5 lg:p-3">
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    [
                      "flex min-h-11 items-center justify-center rounded-control px-3 text-sm font-bold transition lg:justify-start",
                      isActive
        ...
        <div className="flex min-h-0 min-w-0 flex-col">
          <header className="flex h-[72px] shrink-0 items-center justify-between border-b border-[#4d2c1a]/50 bg-gradient-to-r from-[#5f3822] via-[#7a4a2c] to-[#5a321d] px-4 shadow-sm lg:px-7">
            <div className="font-serif text-3xl font-normal tracking-[-0.01em] text-white drop-shadow-sm">
              Paul &amp; Paulina
            </div>
            <NavLink
              to="/notification"
          ...
          <main className="min-h-0 min-w-0 flex-1 overflow-y-auto p-4 lg:p-7">
            <Outlet />
          </main>
        </div>
      </div>
